<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!doctype html>
<meta http-equiv="content-type" content="text/html;charset=UTF-8" />
<head>
<meta charset="utf-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<title>Achivements</title>

<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="apple-touch-icon" href="images/favicon.png">
<link rel="shortcut icon" href="images/favicon.png">
<link rel="stylesheet" href="assets/css/normalize.css">
<link rel="stylesheet" href="assets/css/bootstrap.min.css">
<link rel="stylesheet" href="assets/css/font-awesome.min.css">
<link rel="stylesheet" href="assets/css/themify-icons.css">
<link rel="stylesheet" href="assets/css/pe-icon-7-filled.css">
<link rel="stylesheet" href="assets/css/flag-icon.min.css">
<link rel="stylesheet" href="assets/css/cs-skin-elastic.css">

<link rel="stylesheet" href="assets/css/style.css">
<link href='https://fonts.googleapis.com/css?family=Open+Sans:400,600,700,800' rel='stylesheet' type='text/css'>

</head>
<body>
	<aside id="left-panel" class="left-panel">
		<nav class="navbar navbar-expand-sm navbar-default">
			<div id="main-menu" class="main-menu collapse navbar-collapse">
				<ul class="nav navbar-nav">
					

				<li class="menu-item-has-children dropdown">
<a href="Faculty profile.html"  aria-haspopup="true" aria-expanded="false"> <i class="menu-icon fa fa-table"></i>Faculty profile</a>

</li>
<li class="menu-item-has-children dropdown">
<a href="http://localhost:8085/Profile-Management/ProfessionalActivities_MainPage.jsp"  aria-haspopup="true" aria-expanded="false"> <i class="menu-icon fa fa-table"></i>Professional Activities </a>

</li>
<li class="menu-item-has-children dropdown">
<a href="http://localhost:8085/Profile-Management/Qualification_MainPage.jsp"  aria-haspopup="true" aria-expanded="false"> <i class="menu-icon fa fa-table"></i>Qualification</a>

</li>
<li class="menu-item-has-children dropdown">
<a href="http://localhost:8085/Profile-Management/ServiceDetail_MainPage.jsp"  aria-haspopup="true" aria-expanded="false"> <i class="menu-icon fa fa-table"></i>Service Details</a>

</li>
<li class="menu-item-has-children dropdown">
<a href="http://localhost:8085/Profile-Management/Academic_MainPage.jsp"  aria-haspopup="true" aria-expanded="false"> <i class="menu-icon fa fa-table"></i>Academic Details</a>

</li>
<li class="menu-item-has-children dropdown">
<a href="http://localhost:8085/Profile-Management/Acheivements_MainPage.jsp"  aria-haspopup="true" aria-expanded="false"> <i class="menu-icon fa fa-table"></i>Achivements</a>

</li>
<li class="menu-item-has-children dropdown">
<a href="http://localhost:8085/Profile-Management/RD_MainPage.jsp"  aria-haspopup="true" aria-expanded="false"> <i class="menu-icon fa fa-table"></i>R & D</a>

</li>
<li class="menu-item-has-children dropdown">
<a href="http://localhost:8085/Profile-Management/Responsibility_MainPage.jsp"  aria-haspopup="true" aria-expanded="false"> <i class="menu-icon fa fa-table"></i>Responsibility</a>

</li>
<li class="menu-item-has-children dropdown">
<a href="http://localhost:8085/Profile-Management/Anycontribution.jsp"  aria-haspopup="true" aria-expanded="false"> <i class="menu-icon fa fa-table"></i>Any Contribution</a>

</li>

					

				</ul>
			</div>
		</nav>
	</aside>

<div id="right-panel" class="right-panel">

	<header id="header" class="header">
		<div class="top-left">
		<div class="navbar-header">
		<a class="navbar-brand" href="index.html"><img src="images/logo.png" alt="Logo"></a>
		<a class="navbar-brand hidden" href="index.html"><img src="images/logo2.png" alt="Logo"></a>
		<a id="menuToggle" class="menutoggle"><i class="fa fa-bars"></i></a>
		</div>
		</div>
		
		<div class="top-right">
			<div class="header-menu">
				<div class="header-left">
					
							<a class="nav-link" href="#"><i class="fa fa-power-off"></i>Logout</a>
						</div>
				</div>
			</div>
	
		
	</header>
	
	<div class="breadcrumbs">
		<div class="breadcrumbs-inner">
			<div class="row m-0">
				<div class="col-sm-4">
					<div class="page-header float-left">
						<div class="page-title">
							<h1><strong>Acheivements</strong></h1>
						</div>
					</div>
				</div>
				<div class="col-sm-8">
					<div class="page-header float-right">
						<div class="page-title">
							<ol class="breadcrumb text-right">
								
								<li class="active"> <h4><center></center></h4></li>
							</ol>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
	<div class="content">
		<div class="animated fadeIn">
			<div class="row">
				<div class="col-lg-12">
					<div class="card">
						
						<div class="card-header">
							<div class="card-title"><h4>Workshops and FDP's Attended</h4></strong><br>information about projects funded
						</div>
						<div class="card-body">
							<table class="table">
								<thead class="thead-dark">
									<tr>
										<th scope="col">Type</th>
										<th scope="col">Title</th>
										<th scope="col">Sponsored/Funded</th>
										<th scope="col">Date</th>
										<th scope="col">Place</th>
										<th scope="col">Duration</th>
										<th scope="col">Role</th>
										<th scope="col">National/International</th>
									</tr>								
		
								</thead>
							</table>
						</div>
					</div>
				</div>
			</div>
		</div>
		</div>
		<div class="col-xl-2"><a href="workshop_insert.jsp"><button id="payment-button" type="button" class="btn btn-lg btn-info btn-block">
			<i class=""></i>&nbsp;
			<span id="payment-button-amount">INSERT</span>
			<span id="payment-button-sending" style="display:none;">Sending…</span></button></a>
		 </div>
	</div>
	
	
	<br><br><br><br><br><br>
	<div class="content">
		<div class="animated fadeIn">
			<div class="row">
				<div class="col-lg-12">
					<div class="card">
						
						<div class="card-header">
							<div class="card-title"><h4>Conference Symposia</h4><br>information about conferences and seminars attended
						</div>
						<div class="card-body">
							<table class="table">
								<thead class="thead-dark">
									<tr>
										<th scope="col">Title</th>
										<th scope="col">Place</th>
										<th scope="col">Date</th>
										<th scope="col">Role</th>
										<th scope="col">Type</th>
									</tr>
								</thead>
							</table>
						</div>
					</div>
				</div>
			</div>
		</div>
		</div>
		<div class="col-xl-2"><a href="conference_symosia_insert.jsp"><button id="payment-button" type="button" class="btn btn-lg btn-info btn-block">
			<i class=""></i>&nbsp;
			<span id="payment-button-amount">INSERT</span>
			<span id="payment-button-sending" style="display:none;">Sending…</span></button></a>
		</div>
	</div>


	
	
	
	<br><br><br><br><br><br>
	
	
	<div class="content">
		<div class="animated fadeIn">
			<div class="row">
				<div class="col-lg-12">
					<div class="card">
						
						<div class="card-header">
							<div class="card-title"><h4>Guest Lectures Delivered</h4><br>information about lectures given as a guest lecture outside the college
						</div>
						<div class="card-body">
							<table class="table">
								<thead class="thead-dark">
									<tr>
										<th scope="col">Organization</th>
										<th scope="col">Title</th>
										<th scope="col">Date</th>
									</tr>								
		
								</thead>
							</table>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="col-xl-2"><a href="guest_lecture_deliver_insert.jsp"><button id="payment-button" type="button" class="btn btn-lg btn-info btn-block">
			<i class=""></i>&nbsp;
			<span id="payment-button-amount">INSERT</span>
			<span id="payment-button-sending" style="display:none;">Sending…</span></button></a>
		 </div>
	</div>
	
	<div class="content">
		<div class="animated fadeIn">
			<div class="row">
				<div class="col-lg-12">
					<div class="card">
						
						<div class="card-header">
							<div class="card-title"><h4>Books</h4><br>information about books published</div>
						<div class="card-body">
							<table class="table">
								<thead class="thead-dark">
									<tr>
										<th scope="col">Title</th>
										<th scope="col">Authors</th>
										<th scope="col">Publisher</th>
										<th scope="col">ISBN</th>
										<th scope="col">Year</th>
									</tr>
								</thead>
							</table>
						</div>
					</div>
				</div>
			</div>
		</div>
		</div>
		<div class="col-xl-2"><a href="books_insert.jsp"><button id="payment-button" type="button" class="btn btn-lg btn-info btn-block">
			<i class=""></i>&nbsp;
			<span id="payment-button-amount">INSERT</span>
			<span id="payment-button-sending" style="display:none;">Sending…</span></button></a>
		 </div>
	</div>
	
	
	<div class="content">
		<div class="animated fadeIn">
			<div class="row">
				<div class="col-lg-12">
					<div class="card">
						
						<div class="card-header">
							<div class="card-title"><h4>Books Chapters</h4><br>information about books published
						</div>
						<div class="card-body">
							<table class="table">
								<thead class="thead-dark">
									<tr>
										<th scope="col">Chapter Name</th>
										<th scope="col">Book Name</th>
										<th scope="col">Authors</th>
										<th scope="col">Publisher</th>
										<th scope="col">ISBN</th>
										<th scope="col">Year</th>
									</tr>								
		
								</thead>
							</table>
						</div>
					</div>
				</div>
			</div>
		</div>
		</div>
		<div class="col-xl-2"><a href="books_chapter_insert.jsp"><button id="payment-button" type="button" class="btn btn-lg btn-info btn-block">
			<i class=""></i>&nbsp;
			<span id="payment-button-amount">INSERT</span>
			<span id="payment-button-sending" style="display:none;">Sending…</span></button></a>
		 </div>
	</div>
	
	
	<div class="content">
		<div class="animated fadeIn">
			<div class="row">
				<div class="col-lg-12">
					<div class="card">
						
						<div class="card-header">
							<div class="card-title"><h4>Conference Publications</h4><br>information about paper presented in conference</
						</div>
						<div class="card-body">
							<table class="table">
								<thead class="thead-dark">
									<tr>
										<th scope="col">Title</th>
										<th scope="col">Authors</th>
										<th scope="col">Conference Name</th>
										<th scope="col">Conference Type</th>
										<th scope="col">Organized By</th>
										<th scope="col">Publisher</th>
										<th scope="col">Date</th>
									</tr>
								</thead>
							</table>
						</div>
					</div>
				</div>
			</div>
		</div></div>
		<div class="col-xl-2"><a href="conference_publication_insert.jsp"><button id="payment-button" type="button" class="btn btn-lg btn-info btn-block">
			<i class=""></i>&nbsp;
			<span id="payment-button-amount">INSERT</span>
			<span id="payment-button-sending" style="display:none;">Sending…</span></button></a>
		 </div>
	</div>
	
	
	<div class="content">
		<div class="animated fadeIn">
			<div class="row">
				<div class="col-lg-12">
					<div class="card">
						
						<div class="card-header">
							<div class="card-title"> <h4>Journal Publications</h4><br>information about paper presented in journals
						</div>
						<div class="card-body">
							<table class="table">
								<thead class="thead-dark">
									<tr>
										<th scope="col">Title</th>
										<th scope="col">Journal Name</th>
										<th scope="col">Author1</th>
										<th scope="col">Author2</th>
										<th scope="col">Author3</th>
										<th scope="col">ISSN Print</th>
										<th scope="col">ISSN Online</th>
										<th scope="col">journal Category</th>
										<th scope="col">journal Type</th>
										<th scope="col">Volume Number</th>
										<th scope="col">Page Numbers</th>
										<th scope="col">Issue Number</th>
										<th scope="col">Impact Factor</th>
										<th scope="col">Citation</th>
										<th scope="col">DOI</th>
									</tr>								
		
								</thead>
							</table>
						</div>
					</div>
				</div>
			</div>
		</div>
		</div>
		<div class="col-xl-2"><a href="journal_publication_insert.jsp"><button id="payment-button" type="button" class="btn btn-lg btn-info btn-block">
			<i class=""></i>&nbsp;
			<span id="payment-button-amount">INSERT</span>
			<span id="payment-button-sending" style="display:none;">Sending…</span></button></a>
		 </div>
	</div>
	
	
	<div class="content">
		<div class="animated fadeIn">
			<div class="row">
				<div class="col-lg-12">
					<div class="card">
						
						<div class="card-header">
							<div class="card-title"><h4>Awards</h4><br>information about awards
						</div>
						<div class="card-body">
							<table class="table">
								<thead class="thead-dark">
									<tr>
										<th scope="col">Presented By</th>
										<th scope="col">Awarded For</th>
										<th scope="col">Year</th>
									</tr>
								</thead>
							</table>
						</div>
					</div>
				</div>
			</div>
		</div>
		</div>
		<div class="col-xl-2"><a href="awards_insert.jsp"><button id="payment-button" type="button" class="btn btn-lg btn-info btn-block">
			<i class=""></i>&nbsp;
			<span id="payment-button-amount">INSERT</span>
			<span id="payment-button-sending" style="display:none;">Sending…</span></button></a>
		 </div>
	</div>
	
	
	<div class="content">
		<div class="animated fadeIn">
			<div class="row">
				<div class="col-lg-12">
					<div class="card">
						
						<div class="card-header">
							<div class="card-title"><h4>Technical Talk</h4><br>information about technical talk
						</div>
						<div class="card-body">
							<table class="table">
								<thead class="thead-dark">
									<tr>
										<th scope="col">Title</th>
										<th scope="col">Place</th>
										<th scope="col">Date</th>
									</tr>
								</thead>
							</table>
						</div>
					</div>
				</div>
			</div>
		</div>
		</div>
		<div class="col-xl-2"><a href="technical_talk_insert.jsp"><button id="payment-button" type="button" class="btn btn-lg btn-info btn-block">
			<i class=""></i>&nbsp;
			<span id="payment-button-amount">INSERT</span>
			<span id="payment-button-sending" style="display:none;">Sending…</span></button></a>
		 </div>
	</div>
	
	
	
	<div class="content">
		<div class="animated fadeIn">
			<div class="row">
				<div class="col-lg-12">
					<div class="card">
						
						<div class="card-header">
							<div class="card-title"><h4>Industries Relation</h4><br>information about technical talk
						</div>
						<div class="card-body">
							<table class="table">
								<thead class="thead-dark">
									<tr>
										<th scope="col">Event Name</th>
										<th scope="col">Comapany Name</th>
										<th scope="col">Date</th>
									</tr>
								</thead>
							</table>
						</div>
					</div>
				</div>
			</div>
		</div>
		</div>
		<div class="col-xl-2"><a href="industry_relation_insert.jsp"><button id="payment-button" type="button" class="btn btn-lg btn-info btn-block">
			<i class=""></i>&nbsp;
			<span id="payment-button-amount">INSERT</span>
			<span id="payment-button-sending" style="display:none;">Sending…</span></button></a>
		 </div>
	</div>
	
	
	
	<div class="content">
		<div class="animated fadeIn">
			<div class="row">
				<div class="col-lg-12">
					<div class="card">
						
						<div class="card-header">
							<div class="card-title"><h4>Event Participated</h4><br>information about events participated
						</div>
						<div class="card-body">
							<table class="table">
								<thead class="thead-dark">
									<tr>
										<th scope="col">Event Name</th>
										
										<th scope="col">Date</th>
										<th scope="col">Outside/Inside</th>
										<th scope="col">Place</th>
									</tr>
								</thead>
							</table>
						</div>
					</div>
				</div>
			</div>
		</div>
		</div>
		<div class="col-xl-2"><a href="event_participated_insert.jsp"><button id="payment-button" type="button" class="btn btn-lg btn-info btn-block">
			<i class=""></i>&nbsp;
			<span id="payment-button-amount">INSERT</span>
			<span id="payment-button-sending" style="display:none;">Sending…</span></button></a>
		 </div>
	</div>
	
	
	

	
<div class="clearfix"></div>
</div>


<!-- Footer -->
<footer class="page-footer font-small blue">

  <!-- Copyright -->
  <div class="footer-copyright text-center py-3">© 2020 Copyright:
    <a href="#"> MSRIT</a>
  </div>
  <!-- Copyright -->

</footer>
<!-- Footer -->



<script src="assets/js/vendor/jquery-2.1.4.min.js" type="be41007fad70f509648fc254-text/javascript"></script>
<script src="assets/js/popper.min.js" type="be41007fad70f509648fc254-text/javascript"></script>
<script src="assets/js/plugins.js" type="be41007fad70f509648fc254-text/javascript"></script>
<script src="assets/js/main.js" type="be41007fad70f509648fc254-text/javascript"></script>

<script async src="https://www.googletagmanager.com/gtag/js?id=UA-23581568-13" type="be41007fad70f509648fc254-text/javascript"></script>
<script type="be41007fad70f509648fc254-text/javascript">
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-23581568-13');
</script>
<script src="ajax.cloudflare.com/cdn-cgi/scripts/7089c43e/cloudflare-static/rocket-loader.min.js" data-cf-settings="be41007fad70f509648fc254-|49" defer=""></script></body>
</html>
